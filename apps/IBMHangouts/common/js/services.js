...
hangouts.factory('TagListFactory', function($http) {
		...
		function() {
			var parse = function(eventData) {
				...
				this.getDetailedTime = function() {
					return this.getEventDay() + ' ' + this.getEventFullMonth()
							+ ', ' + this.getEventTime() + ' - '
							+ this.getEndTime();
				};
	...
	return {
		login : function(username, password) {
			...
			return $http.post(restURL + '/login', {
				username : username,
				password : password
			});
		},
		register : function(username, password, name) {
			return $http.post(restURL + '/register', {
				username : username,
				password : password,
				name : name
			});
		...
		update : function(user) {
			...
			return $http.post(restURL + '/updateUser', {
				user : user
			});
		}
	};
	...
	return {
		create : function(event) {
			console.log(event);
			return $http.post(restURL + '/createEvent', {
				event : event
			});
		},
		getEvents : function(filter) {
			return $http.post(restURL + '/getEvents', filter);
		},
